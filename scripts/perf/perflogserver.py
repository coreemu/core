...
# server metrics: loadave1, 5, 15, mem, used cpu% of total, cpu1, cpu2,
# ..., cpun
...
def handler(signum, frame):
    print "stop timestamp:", str(time.time()) + ", cyclecount=", \
           cyclecount, ", caught signal", signum
    sys.exit(0)
...
class ServerMetrics(object):
    ...
    # set values from val = (nump, ldavg1, ldavg5, adavg15, mem, cpu, p1cpu,
    # p2cpu...)
        ...
        if key == "processorusedcputime":
            ...
            for ind in range(0, len(pcpus)):
                ...
                if pcpus[ind] > serverthresholds.getvalue(key):
                    alarm = ["server", os.uname()[1],
                             str(ind) + key, "%.2f" % pcpus[ind],
                             ">", serverthresholds.getvalue(key)]
...
def collectservercputimes():
    # return cpu times in ticks of this server total and each processor
    # 3*(1+#cpu) columns
...
def calcservercputimes(cputimea, cputimeb):
    # return cpu used/total % of this server total and each processor (1+#cpu
    # columns)
    p = {}
...
def csvserverbaseline():
    # return a csv string of raw server metrics data: memfree, memtotal,
    # cpuused, cpusystem, cpuidle
    return "memory (total, free) = " + csvservermems(collectservermems()) + \
        "\ncputime (used, sys, idl) = " + \
        csvservercputimes(collectservercputimes())
...
class LogSession(object):
    ...
    def getpids(self):
        # return dict of all CORE session pids in a dict using node name as
        # the keys
    ...
    def getprocessmetrics(self, pid):
        ...
        # startcode endcode startstack kstkesp signal blocked sigignore
        # sigcatch wchan nswap cnswap exit_signal processor(38) rt_priority
    ...
    def getnodemetrics(self, mindex):
        ...
        for nod in self.pids:
            ...
            if len(processm) > 0:
                ...
                pid = self.pids[nod][0][0]
                nmetric.setvalues((self.getnodethroughput(pid),
                                   # vsize(kb)
                                   sum(map(lambda x: int(x), processm[1])),
                                   # ttime
                                   sum(map(lambda x: int(x), processm[2])),
                                   # utime
                                   sum(map(lambda x: int(x), processm[3])),
                                   # stime
                                   sum(map(lambda x: int(x), processm[4])),
                                   # wtime
                                   sum(map(lambda x: int(x), processm[5]))))
    ...
    def checknodethresholds(self, nname):
        ...
        for keyname in self.nodethresholds.getkeys():
            # print "HIII", keyname, calcm.getvalue(keyname),
            # self.nodethresholds.getvalue(keyname)
            if float(calcm.getvalue(keyname)) > float(
                    self.nodethresholds.getvalue(keyname)):
                # print calculatednodem.getvalue(m)
                alarm = ["node", nname + "/" + self.pids[nname][0][0], keyname,
                         calcm.getvalue(keyname), ">",
                         self.nodethresholds.getvalue(keyname)]
                if options.timestamp:
                    print str(time.time()) + ",",
                print ", ".join(map(lambda x: str(x), alarm))
            ...
            if (nod in nodesa):
                try:
                    if (nodesb[nod] == []) | (nodesa[nod] == []) | \
                       (False is isinstance(nodesb[nod], NodeMetrics)) |  \
                       (False is isinstance(nodesa[nod], NodeMetrics)):
                        if options.timestamp is True:
                            print str(time.time()),
                        print "Warning: nodes %s is not fully instanciated" \
                              % nod
                    else:
                        ...
                        # if nod == 'n6':
                            # print nodesb[nod].getvalue("nodethroughput"),
                            # nodesa[nod].getvalue("nodethroughput")
                        calcm.setvalue("nodethroughput", "%.2f" %
                                       (8 * (nodesb[nod].
                                        getvalue("nodethroughput")
                                        - nodesa[nod].
                                        getvalue("nodethroughput")) /
                                        options.interval))
                        # calc mem node used / host used
                        calcm.setvalue("nodeusedmemory", "%.2f" % (
                            100.0 * (nodesb[nod].getvalue("nodeusedmemory") /
                                     hostusedmem)))
                        ...
                        totalcpu = (nodeb.getvalue("nodetotalcpu") -
                                    nodea.getvalue("nodetotalcpu"))
                        totalcpu = totalcpu / hostusedcpu * 100.0
                        calcm.setvalue("nodetotalcpu", "%.2f" % totalcpu)

                        # calc user cpu time node / host
                        usercpu = (nodeb.getvalue("nodeusercpu") -
                                   nodea.getvalue("nodeusercpu"))
                        usercpu = usercpu / hostusedcpu * 100.0
                        calcm.setvalue("nodeusercpu", "%.2f" % usercpu)

                        # calc system cpu time node / host
                        systemcpu = (nodeb.getvalue("nodesystemcpu") -
                                     nodea.getvalue("nodesystemcpu"))
                        systemcpu = systemcpu / hostusedcpu * 100.0
                        calcm.setvalue("nodesystemcpu", "%.2f" % systemcpu)

                        # calc waitcpu time node / host
                        waitcpu = (nodeb.getvalue("nodewaitcpu") -
                                   nodea.getvalue("nodewaitcpu"))
                        waitcpu = waitcpu / hostusedcpu * 100.0
                        calcm.setvalue("nodewaitcpu", "%.2f" % waitcpu)
...
def main():
    usagestr = "%prog [-h] [options] [args]\n\nLog server and optional " + \
               "CORE session metrics to stdout."
    ...
    parser.add_option("-a", "--alarm", action="store_true",
                      dest="alarm",
                      help="generate alarms based threshold check " +
                           "on each cycle")
    ...
    print "server: ", ", ".\
        join(map(lambda x: str(x), os.uname())), ",", ncpus, "CPU cores"
    print "start timestamp:", time.time(), ", baseline data: "
    print csvserverbaseline()
    print "server metrics: ", ", ".\
        join(map(lambda x: str(x), serverthresholds.getkeys()))
    if options.session is not None:
        print "node metrics: nodename, ", ", ".\
            join(map(lambda x: str(x), logsession.nodethresholds.getkeys()))

...
Also defined here is a TCP based auxiliary server class for supporting
...
class CoreAuxServer(SocketServer.ThreadingMixIn, SocketServer.TCPServer):
    ...
    def __init__(self, server_address, RequestHandlerClass, mainserver):
        self.mainserver = mainserver
        sys.stdout.write(
            "auxiliary server started, listening on: %s:%s\n" %
            server_address)
        sys.stdout.flush()
...
class CoreRequestHandler(SocketServer.BaseRequestHandler):
    ...
    def recvmsg(self):
        ...
        while len(data) < msglen:
            ...
            if len(data) > msglen:
                self.warn("received message length does not match" +
                          "received data " +
                          "(%s != %s)" % (len(data), msglen))
                raise IOError
        try:
    ...
    # when replying to messages from clients
    def dispatchreplies(self, replies, msg):
        '''
        Dispatch replies by CORE to message msg previously received
        from the client.
        '''
    ...
    def handle(self):
        ...
        while True:
            ...
            if (msg.msgtype == coreapi.CORE_API_SESS_MSG):
                # delay is required for brief connections, allow session
                # joining
                time.sleep(0.125)
...
#                     print "XXX XXX XXX referrer:", gc.get_referrers(o)
    ...
    def handlenodemsg(self, msg):
        ...
        if nodexpos is None and nodeypos is None and \
            ...
            nodexpos = int(x)
            nodeypos = int(y)
            # GUI can't handle lat/long, so generate another X/Y position
            # message
            tlvdata = ""
            ...
            self.session.broadcastraw(
                self, coreapi.CoreNodeMessage.pack(
                    0, tlvdata))
        ...
        if msg.flags & coreapi.CORE_API_ADD_FLAG:
            ...
            if msg.flags & coreapi.CORE_API_STR_FLAG:
                ...
                tlvdata += coreapi.CoreNodeTlv.pack(
                    coreapi.CORE_TLV_NODE_NUMBER, nodenum)
    ...
    def handlelinkmsg(self, msg):
        ...
        try:
            if link_msg_type == coreapi.CORE_LINK_WIRELESS:
                ...
                '''
                start = False
                if self.session.getstate() > \
                        coreapi.CORE_EVENT_DEFINITION_STATE:
                    start = True
                ...
                netaddrlist = []
                # print " n1=%s n2=%s net=%s net2=%s" % (node1, node2, net,
                # net2)
                if node1 and net:
                    addrlist = []
        ...
        if msg.flags & coreapi.CORE_API_TTY_FLAG:
            ...
            if msg.flags & coreapi.CORE_API_STR_FLAG or \
               msg.flags & coreapi.CORE_API_TXT_FLAG:
                ...
                if self.verbose:
                    self.info("done exec cmd='%s' with " +
                              "status=%d res=(%d bytes)"
                              % (cmd, status, len(res)))
    ...
    def handleregmsg(self, msg):
        ...
        if gui is None:
            ...
            self.server.setsessionmaster(self)
    ...
    def handlesessionmsg(self, msg):
        ...
        self.info(
            "SESSION message flags=0x%x sessions=%s" %
            (msg.flags, sid_str))
        ...
        if msg.flags == 0:
            ...
            for sid in sids:
                ...
                if session is None:
                    ...
                    continue
                self.info(
                    "request to modify to session %s" %
                    session.sessionid)
...
class BaseAuxRequestHandler(CoreRequestHandler):
    ...
    def setup(self):
        ...
        if self.verbose:
            self.info(
                "new auxiliary server client: %s:%s" %
                self.client_address)
    ...
    def recvmsg(self):
        '''
        Receive data from the client in the supported format. Parse,
        transform to CORE API format and return transformed messages.
    ...
    def dispatchreplies(self, replies, msg):
        '''
        Dispatch CORE 'replies' to a previously received message 'msg' from a
        client.

        Replies passed to this method follow the CORE API.
        This method allows transformation to the form supported by
        the auxiliary handler and within the context of 'msg'.
    ...
    def sendall(self, data):
        ...
        a client. The data is in CORE API format. This method allows
        transformation to the required format supported by this

...
class CoreServices(ConfigurableManager):
    ...
    def setservicefile(self, nodenum, type, filename, srcname, data):
        ...
        if len(type.split(':')) < 2:
            self.session.warn(
                "Received file type did not contain service info.")
            return
    ...
    def handleevent(self, msg):
        ...
        if num > 0:
            for u in unknown:
                ...
                num -= 1
            self.session.warn(
                "Event requested for unknown service(s): %s" %
                udata)
            udata = "Unknown:" + udata

...
class LxBrNet(PyCoreNet):
    ...
    def startup(self):
        ...
        try:
            ...
            # turn off multicast snooping so mcast forwarding occurs w/o IGMP
            # joins
    ...
    def linkconfig(self, netif, bw=None, delay=None,
        ...
        if delay <= 0 and jitter <= 0 and loss <= 0 and duplicate <= 0:
            # possibly remove netem if it exists and parent queue wasn't
            # removed
    ...
    def addrconfig(self, addrlist):
        ...
        for addr in addrlist:
            try:
                check_call(
                    [IP_BIN, "addr", "add", str(addr), "dev", self.brname])
            except Exception as e:
                self.exception(coreapi.CORE_EXCP_LEVEL_ERROR, self.brname,

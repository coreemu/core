...
class XenNode(PyCoreNode):
    ...
    def startup(self):
        self.warn(
            "XEN PVM startup() called: preparing disk for %s" %
            self.name)
        self.lock.acquire()
        try:
            if self.up:
    ...
    def nodefile(self, filename, contents, mode=0o644):
        ...
        if filename in self.FilesRedirection:
            ...
            self.warn(
                "XEN PVM nodefile(filename=%s) redirected to %s" %
    ...
    def cmd(self, args, wait=True):
        ...
        self.warn(
            "XEN PVM cmd(args=[%s]) called, but not yet implemented" %
            cmdAsString)
        return 0
    ...
    def popen(self, args):
        cmdAsString = string.join(args, ' ')
        self.warn(
            "XEN PVM popen(args=[%s]) called, but not yet implemented" %
            cmdAsString)
        return
    ...
    def icmd(self, args):
        cmdAsString = string.join(args, ' ')
        self.warn(
            "XEN PVM icmd(args=[%s]) called, but not yet implemented" %
            cmdAsString)
        return
    ...
    def shcmd(self, cmdstr, sh="/bin/sh"):
        self.warn(
            "XEN PVM shcmd(args=[%s]) called, but not yet implemented" %
            cmdstr)
        return
    ...
    def addfile(self, srcname, filename):
        ...
        if filename in self.FilesRedirection:
            redirFilename = self.FilesRedirection[filename]
            self.warn(
                "XEN PVM addfile(filename=%s) redirected to %s" %
                (filename, redirFilename))
            filename = redirFilename

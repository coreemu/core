...
class DefaultMulticastRouteService(UtilService):
    ...
    def generateconfig(cls, node, filename, services):
        cfg = "#!/bin/sh\n"
        cfg += "# auto-generated by DefaultMulticastRoute service " + \
               "(utility.py)\n"
...
class StaticRouteService(UtilService):
    ...
    def generateconfig(cls, node, filename, services):
        ...
        cfg += "#       Below are samples that you can uncomment and " + \
               "edit.\n#\n"
        for ifc in node.netifs():
            if hasattr(ifc, 'control') and ifc.control is True:
                continue
...
class HttpService(UtilService):
    ...
    def generateapache2conf(cls, node, filename, services):
        ...
        authstr = {cls.APACHEVER22:
                   'LoadModule authz_default_module /usr/lib/apache2/modules' +

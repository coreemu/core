...
_ellipsoid = [
    ...
    # first once is a placeholder only, To allow array indices to
    # match id numbers
    [-1, "Placeholder", 0, 0],
...
def LLtoUTM(ReferenceEllipsoid, Lat, Long, zone=None):
    ...
    UTMNorthing = (k0 * (M+N*tan(LatRad) * (A**2/2 + (5-T+9*C+4*C**2)*A**4/24
                                            + (61 - 58*T + T*T + 600*C
                                               - 330*eccPrimeSquared) *
                                            A**6/720)))
...
Converted to Python by Russ Nelson <nelson@crynwr.com>"""
    ...
    Lat = phi1Rad - (N1*tan(phi1Rad)/R1) * \
        (D*D/2 - (5 + 3*T1 + 10*C1 - 4*C1**2 - 9*eccPrimeSquared) * D*D*D*D/24
         + (61 + 90*T1 + 298*C1 + 45*T1**2 - 252*eccPrimeSquared - 3*C1**2) *
         D**6/720)
    Lat = Lat * _rad2deg

    Long = (D - (1+2*T1+C1) * D**3 / 6 +
            (5 - 2*C1 + 28*T1 - 3*C1**2 + 8*eccPrimeSquared + 24*T1*T1) *
            D**5/120)/cos(phi1Rad)
